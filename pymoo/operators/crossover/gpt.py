...
class GPT(Crossover):
    ...
    def _do(self, _, X, Y,  debug_mode,model_LLM,key,out_filename,parents_obj, **kwargs):
        ...
        st.text_area(f"Prompt:", prompt_content, height=350, key=f"prompt_{self.N_gen}")
        # st.text_area(f"Response:", response, height=50)
        ...
        while retries > 0:
            try:
                ...
                st.text_area(f"Response:", response, height=50, key=f"response_{self.N_gen}")
